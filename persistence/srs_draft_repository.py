    ...
    async def create(
        ...
        *,
        ...
        await self._session.execute(
            text(
                """
                INSERT INTO srs_drafts
                  (id, title, content, source_document_ids, source_snapshot, question, answer, created_by, status, created_at, updated_at)
                VALUES
                  (CAST(:id AS UUID), :title, :content, CAST(:doc_ids AS JSON), CAST(:snapshot AS JSON), :question, :answer, :created_by, 'draft', :created_at, :updated_at)
                """
            ),
            {
                "id": draft_id,
                "title": (title or "").strip() or "SRS Draft",
                "content": content or "",
    ...
    async def update(
        ...
        *,
        ...
        if title is not None:
            updates["title"] = (title or "").strip() or "SRS Draft"
