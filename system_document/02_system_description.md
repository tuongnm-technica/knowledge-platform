...
│  /api/auth  /api/ask  /api/search  /api/docs  /api/pm   │
│  /api/graph /api/models /api/workflows /api/users ...   │
│                                                         │
│  Proxy: /api/sdlc/* ──────────────────────────────────┐ │
└──────┬─────────────────┬────────── ┬───────────────────┼─┘
       │                 │           │                   │
...
| **Zoom** | Pull bản ghi meeting và transcript | Account ID, Client ID/Secret |
| **Google Drive** | Pull tài liệu từ Shared Folder | Service Account JSON |

**Incremental sync:** Mỗi lần chạy, hệ thống chỉ lấy tài liệu mới hơn lần sync cuối. Cơ chế này giúp tối ưu hóa băng thông và tài nguyên CPU cho 30 người dùng đồng thời.
...
Hệ thống cung cấp giao diện quản trị để:
- Đăng ký và quản lý nhiều nhà cung cấp LLM: Ollama, vLLM, OpenAI, Gemini.
- **Model Binding**: Gán các model cụ thể cho các tác vụ khác nhau (ví dụ: Chat dùng Model mạnh, Ingestion dùng Model nhanh).
- Theo dõi trạng thái hoạt động của các Model Server.
