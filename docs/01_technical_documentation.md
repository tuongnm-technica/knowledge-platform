...
> **Phiên bản tài liệu:** 2.0 | **Ngày cập nhật:** 2026-03-30  
> **Trạng thái hệ thống:** Đang phát triển — Internal / Pre-production
...
| ORM (async) | SQLAlchemy + asyncpg | Async session cho mọi route |
...
| Background Jobs | arq | 3 queue riêng biệt |
| Scheduler | APScheduler (sync) | Chạy cùng API process |
| Embedding | Ollama `/api/embed` | Model `bge-m3`, dim 1024, concurrency 5 |
| LLM | Ollama (hoặc bất kỳ OpenAI-compatible) | `/v1/chat/completions`, model `qwen2.5:14b` mặc định |
...
│   └── keyword_index.py           # PostgreSQL fulltext indexing
├── permissions/
...
| `pm_po` | + tasks, drafts, ba-suite, workflows |
| `ba_sa` | + tasks, drafts, ba-suite, workflows |
...
| # | Vấn đề | Mức độ |
|---|---|---|
| 1 | `JWT_SECRET="change-me-in-production"` mặc định — cực kỳ nguy hiểm nếu deploy | **Nghiêm trọng** |
| 2 | CORS `allow_origins=["*"]` — không phù hợp production | **Cao** |
| 3 | PostgreSQL không có trong docker-compose — phải cài riêng | Trung bình |
| 4 | Không có test suite (unit/integration) được xác nhận | **Cao** |
| 5 | SDLC step cards trong ba-suite dùng time-based simulation (hardcode `attempts===5,15,25`) | Thấp |
| 6 | Ollama không có cloud fallback — nếu Ollama down, toàn bộ AI đình trệ | Trung bình |
| 7 | Reranker cache là **in-memory dict** — mất khi restart, không share giữa workers | Thấp |
| 8 | Vision mặc định `VISION_ENABLED=False` — cần bật thủ công và cài vision model | Thấp |
| 9 | `AGENT_LOGIC_CHECK_ENABLED` thêm 1 LLM call — tăng latency đáng kể | Trung bình |
