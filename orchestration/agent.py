...
class OllamaLLM:
    ...
    async def chat(self, system: str, user: str, max_tokens: int = 800) -> str:
        ...
        try:
            ...
            raise RuntimeError(
                f"Ollama timed out ({settings.LLM_TIMEOUT}s)"
            )
...
class Agent:
    ...
    async def search(self, query: SearchQuery) -> list[SearchResult]:
        ...
        allowed_ids = await self._permissions.allowed_docs(
            query.user_id
        )
        ...
        doc_ids = list({
            str(r["document_id"]) for r in raw
        })
    ...
    async def _fetch_doc_meta(self, doc_ids):
    ...
    def _to_results(self, scored, doc_meta):
